      ...
      <div className="max-w-[1280px] mx-auto h-full flex items-center justify-between px-6">
        ...
        <div className="hidden sm:flex items-center gap-6">
          <button onClick={() => navigate("/")} className="relative text-sm font-medium transition-colors duration-150 cursor-pointer"
            style={{ color: isDashboard ? "#0D9488" : "#64748B", fontWeight: isDashboard ? 600 : 500 }}>
            任务面板
            {isDashboard && <motion.div layoutId="nav-underline" className="absolute -bottom-[18px] left-0 right-0 h-0.5 bg-[#14B8A6]" transition={{ duration: 0.25 }} />}
          </button>
          <button onClick={() => navigate("/history")} className="relative text-sm font-medium transition-colors duration-150 cursor-pointer"
            style={{ color: isHistory ? "#0D9488" : "#64748B", fontWeight: isHistory ? 600 : 500 }}>
            更新历史
            {isHistory && <motion.div layoutId="nav-underline" className="absolute -bottom-[18px] left-0 right-0 h-0.5 bg-[#14B8A6]" transition={{ duration: 0.25 }} />}
          </button>
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <button onClick={onToggleDigest} className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#F1F5F9] hover:bg-[#E2E8F0] transition-colors duration-150 cursor-pointer">
            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: dailyDigestEnabled ? "#14B8A6" : "#CBD5E1" }} />
          ...
          <motion.button whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.98 }}
            onClick={onNewTask}
            className="flex items-center gap-1.5 px-5 py-2 bg-[#14B8A6] text-white text-sm font-semibold rounded-lg h-9 hover:bg-[#2DD4BF] transition-colors duration-150 cursor-pointer">
      ...
      {mobileMenuOpen && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
          ...
          <button onClick={() => { navigate("/"); setMobileMenuOpen(false); }} className="flex items-center gap-2 text-sm font-medium py-2 cursor-pointer"
            style={{ color: isDashboard ? "#0D9488" : "#64748B" }}>
          ...
          <button onClick={() => { navigate("/history"); setMobileMenuOpen(false); }} className="flex items-center gap-2 text-sm font-medium py-2 cursor-pointer"
            style={{ color: isHistory ? "#0D9488" : "#64748B" }}>
          ...
          <div className="border-t border-[#E2E8F0] pt-3 flex flex-col gap-3">
            ...
            <button onClick={() => { onNewTask(); setMobileMenuOpen(false); }}
              className="flex items-center justify-center gap-1.5 px-5 py-2.5 bg-[#14B8A6] text-white text-sm font-semibold rounded-lg cursor-pointer">
